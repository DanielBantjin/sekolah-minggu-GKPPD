<x-app-layout>
    <div class="p-6">
        ...
        <form method="POST" action="{{ route('admin.students.update', $student) }}" class="mt-4 space-y-3">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-sm font-medium">Nama</label>
                <input class="border rounded w-full p-2" name="name" value="{{ old('name', $student->name) }}" required>
            </div>
