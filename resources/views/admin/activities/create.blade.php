<x-app-layout>
    <div class="p-6">
        <h1 class="text-3xl font-bold mb-6">Tambah Kegiatan Baru</h1>

        <form method="POST" action="{{ route('admin.activities.store') }}" class="bg-white rounded-lg shadow p-6">
            @csrf
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                              class="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Jelaskan detail kegiatan (opsional)">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="category" id="category"
                        ...
                        <option value="Kelas" {{ old('category') === 'Kelas' ? 'selected' : '' }}>Kelas Kelompok</option>
                        <option value="Renungan" {{ old('category') === 'Renungan' ? 'selected' : '' }}>Renungan Bersama</option>
                        <option value="Games" {{ old('category') === 'Games' ? 'selected' : '' }}>Games / Permainan</option>
                        <option value="Doa" {{ old('category') === 'Doa' ? 'selected' : '' }}>Doa & Saksian</option>
